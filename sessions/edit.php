<?php
    ...
    $session_id = $title = $type = $speaker = $room = $cost = $start = $end = $description = "";
    ...
    if(isset($_GET['session_id'])){
        ...
        $row = sqlSelect("*","sessions","id = '$session_id'"); 
        $room = $row['room_id'];
        $title = $row['title'];
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if((empty($type_err) && empty($title_err) && empty($speaker_err)) && (empty($room_err)&&empty($cost_err)&&empty($start_err))&&(empty($end_err)&&empty($description_err))){
            ...
            // $end = strtotime($end);
            $sql = sqlSelectReturnQuery("sessions.id","sessions","sessions.id = '$session_id'");
            $sql = sqlSelectReturnQuery("sessions.id, sessions.start, sessions.end","rooms, sessions","rooms.id=sessions.room_id AND rooms.id = '$room'AND sessions.start  <= '$start' AND '$end' <= sessions.end");
            $i=1;
                ...
                endwhile;
            $i=0;
            $num_rows = mysqli_num_rows($sql) - $result;
           
            if($num_rows>0){
                $_SESSION['status'] = "Room already Booked this time ";
                $start_err = $_SESSION['status'];
                $end_err = $_SESSION['status'];
            }
            else{
            $_SESSION['status'] = "Session successfully Edited ";
            sqlUpdate("sessions", "room_id = '$room',title = '$title',description = '$description',speaker = '$speaker', start = '$start', end = '$end', type = '$type',cost = $cost", "id = $session_id");
            ...
            <form class="needs-validation" novalidate action="" method="POST">
                ...
                <div class="row">
                    <div class="col-12 col-lg-4 mb-3">
                        ...
                        <input type="number" class="form-control <?php if(!empty($speaker_err)){ echo 'is-invalid';} ?>" id="inputCost" name="cost" placeholder="" value="<?php if(!empty($cost)){ echo $cost;} ?>">
                        <div class="invalid-feedback">
                            <?php echo $cost_err; ?>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-12 col-lg-6 mb-3">
                        ...
                        <input type="text"
                               class="form-control <?php if(!empty($speaker_err)){ echo 'is-invalid';} ?>"
                               id="inputStart"
                               name="start"
                ...
                <div class="row">
                    <div class="col-12 mb-3">
                        ...
                        <textarea class="form-control <?php if(!empty($speaker_err)){ echo 'is-invalid';} ?>" id="textareaDescription" name="description" placeholder="" rows="5"><?php if(!empty($description)){ echo $description;} ?></textarea>
                        <div class="invalid-feedback">
